...
| §3.8: signing keys **"injected at runtime from a managed secrets store"** | Keys are Railway service-scoped variables, injected at runtime and never committed. There is no separate vault integration | Testnet only (D-7) | Accepted as delivered |
...
| D-7 | Which network does the sprint deliver on? | **Stellar testnet only.** The mainnet preflight (#50) and smoke payout (#51) are out of scope | 25 Sep |
